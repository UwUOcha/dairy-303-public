...
async function workerHarness({ legacy = false, failPath = "" } = {}) {
  ...
  return {
    ...
    fetches: () => fetches,
    cache: async () => caches.open((await caches.keys()).find(name => name === 'mp-shell-v26')),
    async get(path) {
      let result;
...
test('a fully installed update replaces a retained old controller and removes only old shell caches', async () => {
  ...
  assert.deepEqual((await worker.names()).sort(), ['mp-shell-v26', 'unrelated-cache']);
  assert.equal(worker.version(), 'mp-shell-v26');
  worker.offline();
  worker.restart();
...
test('closed beta worker removes legacy shells and never serves cached pages', async () => {
  ...
  vm.runInNewContext(await readFile(new URL('beta-sw.js', assets), 'utf8'), {
    caches: { keys: async () => ['mp-shell-v26', 'unrelated-cache'], delete: async k => removed.push(k) },
  ...
  assert.deepEqual(removed, ['mp-shell-v26']); assert.equal(handlers.fetch, undefined);
});
